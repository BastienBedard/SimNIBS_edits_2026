...
#Uses the same comparison logic as the other scripts, 
# but compares the electric potential V instead of E or J.
...
# Electric potential has an arbitrary constant offset.
# If True, remove the volume-weighted mean from both reference V and test V
# before computing the relative error.
REMOVE_V_OFFSET = True
...
def find_voltage_field_name(mesh):
    """
    Finds the electric potential field in the .msh file.

    SimNIBS commonly stores electric potential as 'v', not 'V'.
    ...
    for name in possible_names:
        if name in mesh.field:
            return name

    available_fields = list(mesh.field.keys())

    raise KeyError(
        "Could not find an electric potential field named 'v' or 'V'.\n"
        f"Available fields are: {available_fields}\n\n"
        "If your file does not contain electric potential, make sure your "
        "simulation is saving the potential field."
    )
...
def get_tetra_data(mesh,
    ...
    voltage_field_name = find_voltage_field_name(mesh)
    V_all = mesh.field[voltage_field_name].value

    # Make sure V is a 1D nodal scalar array
    V_all = np.asarray(V_all).squeeze()

    if V_all.ndim != 1:
        raise ValueError(
            f"Voltage field '{voltage_field_name}' was found, but it does not look "
            f"like a 1D scalar field. Its shape is: {V_all.shape}"
        )

    # Method A:
    # Assign one V value to each tetrahedron by averaging the V values
    # at the four tetrahedron nodes.
    V_tet = np.mean(V_all[tet_nodes], axis=1)

    # Filter tissue tags
    tags_tet = tags_tet_all[tissue_mask]

    return centers, volumes, V_tet, tags_tet
...
def estimate_test_V_with_knn(reference_centers,
                             reference_tags,
                             test_centers,
                             test_V,
                             test_tags,
                             ...
                             match_tissue_tags):
    """
    Estimates the test electric potential at the reference tetrahedron centers.
    ...
    The test scalar field is estimated using distance-weighted k-nearest neighbours.
    """

    test_V_on_reference = np.zeros(len(reference_centers))
    ...
    if match_tissue_tags:
        ...
        for tag in unique_reference_tags:
            ...
            # Electric potentials of the neighboring test tetrahedra
            neighboring_V = test_V_tag[indices]

            # Distance-weighted estimate of the test scalar field
            estimated_V = np.sum(weights * neighboring_V, axis=1)

            test_V_on_reference[ref_mask] = estimated_V
        ...
        neighboring_V = test_V[indices]
        estimated_V = np.sum(weights * neighboring_V, axis=1)

        test_V_on_reference = estimated_V
        valid_reference[:] = True

    return test_V_on_reference, valid_reference


def remove_volume_weighted_mean(V, volumes):
    """
    Removes the volume-weighted mean from a scalar field.

    This is useful for electric potential because V has an arbitrary constant offset.
    """

    mean_V = np.sum(V * volumes) / np.sum(volumes)

    return V - mean_V


def relative_scalar_error(reference_centers,
                          reference_volumes,
                          reference_V,
                          reference_tags,
                          test_centers,
                          ...
                          remove_v_offset=REMOVE_V_OFFSET):
    """
    Compares test_V to reference_V.
    ...
    test_V_on_reference, valid_reference = estimate_test_V_with_knn(
    ...
    reference_V_valid = reference_V[valid_reference]
    test_V_valid = test_V_on_reference[valid_reference]
    reference_volumes_valid = reference_volumes[valid_reference]

    if len(reference_V_valid) == 0:
        raise ValueError("No valid reference points were available for comparison.")

    # Remove arbitrary voltage offset before comparison
    if remove_v_offset:
        reference_V_valid = remove_volume_weighted_mean(
            reference_V_valid,
            reference_volumes_valid
        )

        test_V_valid = remove_volume_weighted_mean(
            test_V_valid,
            reference_volumes_valid
        )

    # Scalar difference
    diff = reference_V_valid - test_V_valid

    # Squared scalar values
    diff_squared = diff**2
    reference_squared = reference_V_valid**2
    ...
    if denominator == 0:
        raise ZeroDivisionError(
            "The reference voltage field has zero norm after offset removal. "
            "Cannot compute a relative error."
        )
...
for file in files:
    ...
    centers, volumes, V, tags = get_tetra_data(
        mesh,
        compare_only_list=COMPARE_ONLY_LIST,
        used_tags=USED_TAGS
    )

    tetra_counts.append(len(V))
    centers_list.append(centers)
    volumes_list.append(volumes)
    V_list.append(V)
    tags_list.append(tags)

    print("Tetrahedra kept:", len(V))
...
if REFERENCE_MODE == "most_tetrahedra":
    ...
    reference_V = V_list[reference_index]
    ...
    print("Remove V offset:", REMOVE_V_OFFSET)
    print()
...
if REFERENCE_MODE in ["most_tetrahedra", "specific_file"]:
    ...
    for i, file in enumerate(files):
        ...
        print(f"  Relative electric-potential scalar-field error: {error:.6f} %")
        print()
...
if REFERENCE_MODE == "next_finer":
    plt.ylabel("Relative electric-potential difference to next finer mesh (%)")
    reference_label = "next finer mesh"
elif REFERENCE_MODE == "specific_file":
    plt.ylabel("Relative electric-potential error compared to reference (%)")
...
if COMPARE_ONLY_LIST:
    plt.title(f"Electric-potential difference in chosen tissue types\nReference: {reference_label}")
else:
    plt.title(f"Electric-potential difference\nReference: {reference_label}")
...
graph_data_filename = (
    f"meshcompare_V_{tissue_label}_ref_{reference_name_for_file}_{timestamp}.csv"
)
...
with open(graph_data_path, "w", newline="") as csvfile:
    ...
    for tetra_count, relative_error, test_name, ref_name in zip(
        ...
        relative_errors_sorted,
        file_names_sorted,
        reference_file_names_for_plot[sort_indices]
    ):
        writer.writerow([
            tetra_count,
            relative_error,
            ...
            REMOVE_V_OFFSET,
            "V",
            "average_nodal_V_per_tetrahedron"
        ])
