...
#Usually for comparing more than two meshes, will output a line graph for all meshes
...
def get_tetra_data(mesh,
    ...
    return centers, volumes, E_tet, tags_tet
...
for file in files:
    ...
    centers, volumes, E, tags = get_tetra_data(
        mesh,
        compare_only_gray_white=COMPARE_ONLY_LIST,
        gray_white_tags=USED_TAGS
    )
...
if REFERENCE_MODE in ["most_tetrahedra", "specific_file"]:
    ...
    for i, file in enumerate(files):
        ...
        print(f"  Relative vector-field error: {error:.6f} %")
        print()
